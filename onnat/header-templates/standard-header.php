...
if( !class_exists( 'Onnat_Theme_Standard_Header' ) ) {

	/**
	 * The Onnat Theme Standard Header setup class.
	 */
    class Onnat_Theme_Standard_Header {
        ...
        public function build( $args ) {
			...
			if( 'header' === $args['header_type'] && 'standard_header' === $args['header_id'] ) {

				$full_width = kinfw_onnat_theme_options()->kinfw_get_option( 'standard_header_full_width' );
				...
				printf( '<!-- #kinfw-masthead -->
					<header id="kinfw-masthead" class="kinfw-std-header">
						<div class="%1$s">
							%5$s
					...
					kinfw_onnat_theme_header()->menu_block( kinfw_onnat_theme_options()->kinfw_get_option( 'standard_header_menu' ), 'lg' ),
					...
					kinfw_onnat_theme_header()->menu_block( kinfw_onnat_theme_options()->kinfw_get_option( 'standard_header_menu' ), 'mobile' ),
				);
			}
				...
				switch( $action ) {
					...
					case 'hamburger-button':
						add_action( 'wp_footer', function(){
							$page_id = kinfw_onnat_theme_options()->kinfw_get_option( 'standard_header_icon_hamburger_button' );
...
if( !function_exists( 'kinfw_onnat_theme_standard_header' ) ) {

    /**
     * Returns the instance of a class.
     */
    function kinfw_onnat_theme_standard_header( $args ) {

        return Onnat_Theme_Standard_Header::get_instance( $args );
    }
}

kinfw_onnat_theme_standard_header( $args );
