...
$page_settings = kinfw_onnat_theme_blog_utils()->layout( 'front_posts_page_template', 'front_posts_page_sidebars' );
    ...
    <div id="primary" class="kinfw-content-holder">
        <?php
            ...
            if( have_posts() ):
                ...
                $post_style = kinfw_onnat_theme_options()->kinfw_get_option( 'front_posts_page_post_style' );
                $classes    = '';

                if( in_array( $post_style, [ 'grid-1', 'grid-2', 'grid-3', 'grid-4' ] ) ) {

                    $column = kinfw_onnat_theme_options()->kinfw_get_option( 'front_posts_grid' );
                    $size   = $column['size'];
