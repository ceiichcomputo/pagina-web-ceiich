...
$page_settings = kinfw_onnat_theme_blog_utils()->layout( 'cpt_kinfw_team_member_archive_template', 'cpt_kinfw_team_member_archive_sidebars' );
    ...
    <div id="primary" class="kinfw-content-holder">
        <?php
            ...
            if( have_posts() ):
                ...
                $post_style = kinfw_onnat_theme_options()->kinfw_get_option( 'cpt_kinfw_team_member_archive_post_style' );
                $post_style = !empty( $post_style ) ? $post_style : 'grid-1';

                $classes    = '';

                if( in_array( $post_style, [ 'grid-1', 'grid-2', 'grid-3', 'grid-4', 'grid-5' ] ) ) {

                    $column = kinfw_onnat_theme_options()->kinfw_get_option( 'cpt_kinfw_team_member_archive_item_per_row' );
                ...
                echo '<div class="kinfw-row kinfw-team-group">';
                ...
                while( have_posts() ):
                    ...
                    get_template_part(
                        'template-parts/archive/kinfw-team-member/'. $post_style . '/content',
                        '',
                        [ 'classes' => $classes ]
                    );
